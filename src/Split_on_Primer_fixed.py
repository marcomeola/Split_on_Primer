...
def extract_sequences():
    ...
    for headers in lines:
        header = headers.__next__().strip()
        if file_format == '>': sequence = [''.join(line.strip() for line in lines.__next__()).upper()]
        else:
            temporary_list, sequence, quality = [line.strip() for line in lines.__next__()], [], []
            ...
            while len(quality) < len(sequence):
                if len(quality) == 0 and len(sequence) == 1:
                    quality.append(headers.__next__().strip())
                else:
                    quality += [line.strip() for line in lines.__next__()]
...
def find_best_primer(inputs):
    read, primer_list, size = inputs
